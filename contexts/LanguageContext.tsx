...
import { fetchContent, buildTranslations } from '@/lib/cms'
...
// Static fallback translations (used until Supabase loads)
...
export function LanguageProvider({ children }: { children: ReactNode }) {
  ...
  const [cmsTranslations, setCmsTranslations] = useState<{
    en: Record<string, string>
    it: Record<string, string>
  } | null>(null)

  // Fetch translations from Supabase on mount
  useEffect(() => {
    fetchContent()
      .then((content) => {
        if (content.length > 0) {
          setCmsTranslations({
            en: buildTranslations(content, 'en'),
            it: buildTranslations(content, 'it'),
          })
        }
      })
      .catch(() => {
  ...
  const t = (key: string): string => {
    // Prioritize CMS translations, fall back to static
    const translations = cmsTranslations ?? staticTranslations
    return translations[language]?.[key] ?? staticTranslations[language]?.[key] ?? key
  }
